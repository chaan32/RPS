...
| YOLO-Pose | `model/yolo/yolo11s-pose.pt` | 최종 3개 시나리오의 `ground_truth.csv`와 `diagnostics/fusion/fusion_risk.csv` 비교 | 공식 COCO keypoint AP가 아니라, worker가 Pose -> Homography -> BEV/fusion 입력까지 유지됐는지 보는 operational metric |
