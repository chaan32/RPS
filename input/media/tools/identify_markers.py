"""ArUco 마커 ID 시각 확인 유틸.

이미지에 감지된 ArUco 마커의 ID 를 큼지막하게 라벨링해서 반환.
어떤 마커가 어느 코너에 있는지 눈으로 확인하거나, 서버 API
(/aruco/identify) 응답 이미지를 만들 때 사용한다.

CLI 사용법:
    python -m input.media.tools.identify_markers --image calibration/test_cam1.jpg

API 사용법 (server/main.py 의 /aruco/identify 엔드포인트):
